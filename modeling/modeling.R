...
### AIDS Model Implementation ###

# Prepare data for AIDS estimation

ssu_aids_data <- ssu_with_prices %>%
  mutate(
    w_tobacco_alcohol = (exp_tobacco_real + exp_alcohol_real) /
      exp_total_consumption_real,
    # Ensure shares sum to 1
    w_food_adj = w_food / (w_food + w_tobacco_alcohol + w_other),
    w_ta_adj = w_tobacco_alcohol / (w_food + w_tobacco_alcohol + w_other),
    w_other_adj = w_other / (w_food + w_tobacco_alcohol + w_other),
    # Create price index (Stone price index)
    log_price_index = w_food_adj *
      log(100) + # Normalized food price
      w_ta_adj * log_price_ta +
      w_other_adj * log(100) # Normalized other price
  ) %>%
  filter(
    !is.na(w_tobacco_alcohol) &
      w_tobacco_alcohol > 0 &
      !is.na(log_price_ta) &
      is.finite(log_price_ta) &
      !is.na(log_exp_total) &
      is.finite(log_exp_total)
  )

unicef_aids_data <- unicef_with_prices %>%
  mutate(
    # Ensure shares sum to 1
    w_food_adj = w_food / (w_food + w_tobacco_alcohol + w_other),
    w_ta_adj = w_tobacco_alcohol / (w_food + w_tobacco_alcohol + w_other),
  ...
  filter(!is.na(w_tobacco_alcohol) & w_tobacco_alcohol > 0)

# LA-AIDS model using systemfit
# System of equations for budget shares
aids_formulas <- list(
  w_food_adj ~ log_price_ta + I(log_exp_total - log_price_index),
  w_ta_adj ~ log_price_ta + I(log_exp_total - log_price_index)
  # w_other is omitted to avoid singularity
)

# Estimate the system
aids_model_ssu <- systemfit(
  aids_formulas,
  method = "SUR",
  data = ssu_aids_data,
  restrict.matrix = NULL # Can add homogeneity restrictions here
)

aids_model_unicef <- systemfit(
  aids_formulas,
  method = "SUR",
  data = unicef_aids_data,
  restrict.matrix = NULL
)

# Extract elasticities from AIDS model
aids_elasticities <- function(model, data, share_vars) {
  # Get coefficients
  coefs <- coef(model)

  # Calculate mean shares
  mean_shares <- colMeans(data[share_vars], na.rm = TRUE)

  # Price elasticity for tobacco+alcohol
  # Note: systemfit uses eq1_, eq2_ etc. naming convention
  # eq1 corresponds to w_food_adj, eq2 corresponds to w_ta_adj
  gamma_ta <- coefs["eq2_log_price_ta"]
  beta_ta <- coefs["eq2_I(log_exp_total - log_price_index)"]

  # Uncompensated price elasticity
  price_elast_ta <- -1 +
    (gamma_ta - beta_ta * mean_shares["w_ta_adj"]) / mean_shares["w_ta_adj"]

  # Income elasticity
  income_elast_ta <- 1 + beta_ta / mean_shares["w_ta_adj"]

  list(
    price_elasticity = price_elast_ta,
    income_elasticity = income_elast_ta
  )
}


aids_results_ssu <- aids_elasticities(
  aids_model_ssu,
  ssu_aids_data,
  c("w_food_adj", "w_ta_adj", "w_other_adj")
)

aids_results_unicef <- aids_elasticities(
  aids_model_unicef,
  unicef_aids_data,
  c("w_food_adj", "w_ta_adj", "w_other_adj")
)

aids_comparison <- data.frame(
  Year = c("2021", "2024"),
  AIDS_Price_Elasticity = c(
    aids_results_ssu$price_elasticity,
    aids_results_unicef$price_elasticity
  ),
  AIDS_Income_Elasticity = c(
    aids_results_ssu$income_elasticity,
    aids_results_unicef$income_elasticity
  ),
  Change_Price = c(
    NA,
    aids_results_unicef$price_elasticity - aids_results_ssu$price_elasticity
  ),
  Change_Income = c(
    NA,
    aids_results_unicef$income_elasticity - aids_results_ssu$income_elasticity
  )
)

print("\nAIDS Model Comparison 2021 vs 2024:")
print(aids_comparison)
