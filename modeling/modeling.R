...
### Deaton Spatial Variation Method Implementation ###

ssu_cluster_data <- ssu_with_prices %>%
  filter(!is.na(log_price_tobacco) & !is.na(log_price_alcohol)) %>%
  group_by(region, period_id) %>%
  mutate(
    # Calculate cluster (region-period) averages
    cluster_avg_log_exp = mean(log_exp_total, na.rm = TRUE),
    cluster_avg_w_tobacco = mean(w_tobacco, na.rm = TRUE),
    cluster_avg_w_alcohol = mean(w_alcohol, na.rm = TRUE),
    cluster_avg_w_ta = mean(w_tobacco_alcohol, na.rm = TRUE),

    # Calculate deviations from cluster means
    dev_log_exp = log_exp_total - cluster_avg_log_exp,
    dev_w_tobacco = w_tobacco - cluster_avg_w_tobacco,
    dev_w_alcohol = w_alcohol - cluster_avg_w_alcohol,
    dev_w_ta = w_tobacco_alcohol - cluster_avg_w_ta
  ) %>%
  ungroup()

# Prepare UNICEF data similarly
unicef_cluster_data <- unicef_with_prices %>%
  filter(!is.na(log_price_ta)) %>%
  group_by(region) %>% # UNICEF only has one period
  mutate(
    cluster_avg_log_exp = mean(log_exp_total, na.rm = TRUE),
    cluster_avg_w_ta = mean(w_tobacco_alcohol, na.rm = TRUE),
    dev_log_exp = log_exp_total - cluster_avg_log_exp,
...
### Stage 1: Within-cluster demand equations (removing cluster effects) ###

# For tobacco (SSU only)
tobacco_within <- lm(
  dev_w_tobacco ~
    dev_log_exp +
      settlement_type +
      ...
      head_educ +
      n_smokers,
  data = ssu_cluster_data,
  weights = weight
)

# For alcohol (SSU only)
alcohol_within <- lm(
  dev_w_alcohol ~
    dev_log_exp +
      settlement_type +
...
# For tobacco & alcohol combined (SSU)
ta_within_ssu <- lm(
  dev_w_ta ~
    dev_log_exp +
      settlement_type +
...
# For tobacco & alcohol combined (UNICEF)
ta_within_unicef <- lm(
  dev_w_ta ~
    dev_log_exp +
      settlement_type +
      ...
      head_educ +
      is_idp,
  data = unicef_cluster_data,
  weights = weight
)


### Stage 2: Between-cluster analysis ###

# Create cluster-level data for SSU
ssu_clusters <- ssu_cluster_data %>%
  group_by(region, period_id) %>%
  summarise(
    # Cluster means
    mean_w_tobacco = weighted.mean(w_tobacco, weight, na.rm = TRUE),
    mean_w_alcohol = weighted.mean(w_alcohol, weight, na.rm = TRUE),
    mean_w_ta = weighted.mean(w_tobacco_alcohol, weight, na.rm = TRUE),
    mean_log_exp = weighted.mean(log_exp_total, weight, na.rm = TRUE),

    # Prices (already at cluster level)
    log_price_tobacco = first(log_price_tobacco),
    log_price_alcohol = first(log_price_alcohol),
    log_price_ta = first(log_price_ta),

    # Cluster size for weighting
    n_obs = n(),
    total_weight = sum(weight),
    .groups = 'drop'
  )

# Create cluster-level data for UNICEF
unicef_clusters <- unicef_cluster_data %>%
  group_by(region) %>%
  summarise(
    mean_w_ta = weighted.mean(w_tobacco_alcohol, weight, na.rm = TRUE),
    mean_log_exp = weighted.mean(log_exp_total, weight, na.rm = TRUE),
...
### Calculate Deaton elasticities ###

# Income elasticity formula: (beta / mean_share) + 1
# where beta is coefficient on log expenditure

# Price elasticity using cluster-level variation
# Following Deaton's approach: correlate cluster budget shares with cluster prices

# Tobacco elasticities (SSU)
tobacco_cluster_reg <- lm(
  mean_w_tobacco ~ log_price_tobacco + mean_log_exp,
  data = ssu_clusters,
  weights = total_weight
)

tobacco_income_elast <- (coef(tobacco_within)["dev_log_exp"] /
  mean(ssu_cluster_data$w_tobacco, na.rm = TRUE)) +
  1

tobacco_price_elast <- coef(tobacco_cluster_reg)["log_price_tobacco"] /
  mean(ssu_clusters$mean_w_tobacco, na.rm = TRUE)

# Alcohol elasticities (SSU)
alcohol_cluster_reg <- lm(
  mean_w_alcohol ~ log_price_alcohol + mean_log_exp,
  data = ssu_clusters,
  weights = total_weight
)

alcohol_income_elast <- (coef(alcohol_within)["dev_log_exp"] /
  mean(ssu_cluster_data$w_alcohol, na.rm = TRUE)) +
  1

alcohol_price_elast <- coef(alcohol_cluster_reg)["log_price_alcohol"] /
  mean(ssu_clusters$mean_w_alcohol, na.rm = TRUE)

# Tobacco & Alcohol combined elasticities (SSU)
ta_cluster_reg_ssu <- lm(
  mean_w_ta ~ log_price_ta + mean_log_exp,
  data = ssu_clusters,
  weights = total_weight
)

ta_income_elast_ssu <- (coef(ta_within_ssu)["dev_log_exp"] /
  mean(ssu_cluster_data$w_tobacco_alcohol, na.rm = TRUE)) +
  1

ta_price_elast_ssu <- coef(ta_cluster_reg_ssu)["log_price_ta"] /
  mean(ssu_clusters$mean_w_ta, na.rm = TRUE)

# Tobacco & Alcohol combined elasticities (UNICEF)
ta_cluster_reg_unicef <- lm(
  mean_w_ta ~ log_price_ta + mean_log_exp,
  data = unicef_clusters,
  weights = total_weight
)

ta_income_elast_unicef <- (coef(ta_within_unicef)["dev_log_exp"] /
  mean(unicef_cluster_data$w_tobacco_alcohol, na.rm = TRUE)) +
  1

ta_price_elast_unicef <- coef(ta_cluster_reg_unicef)["log_price_ta"] /
  mean(unicef_clusters$mean_w_ta, na.rm = TRUE)


### Calculate standard errors using delta method ###

# For tobacco
tobacco_se_income <- deltaMethod(
  tobacco_within,
  "dev_log_exp / b0 + 1",
  parameterNames = c("b0" = "dev_log_exp"),
  constants = list(b0 = mean(ssu_cluster_data$w_tobacco, na.rm = TRUE))
)

tobacco_se_price <- sqrt(vcov(tobacco_cluster_reg)[
  "log_price_tobacco",
  "log_price_tobacco"
]) /
  mean(ssu_clusters$mean_w_tobacco, na.rm = TRUE)
...
# For alcohol
alcohol_se_income <- deltaMethod(
  alcohol_within,
  "dev_log_exp / b0 + 1",
  parameterNames = c("b0" = "dev_log_exp"),
  constants = list(b0 = mean(ssu_cluster_data$w_alcohol, na.rm = TRUE))
)

alcohol_se_price <- sqrt(vcov(alcohol_cluster_reg)[
  "log_price_alcohol",
  "log_price_alcohol"
]) /
  mean(ssu_clusters$mean_w_alcohol, na.rm = TRUE)
...
# For combined SSU
ta_ssu_se_income <- deltaMethod(
  ta_within_ssu,
  "dev_log_exp / b0 + 1",
  parameterNames = c("b0" = "dev_log_exp"),
  constants = list(b0 = mean(ssu_cluster_data$w_tobacco_alcohol, na.rm = TRUE))
)

ta_ssu_se_price <- sqrt(vcov(ta_cluster_reg_ssu)[
  "log_price_ta",
  "log_price_ta"
]) /
  mean(ssu_clusters$mean_w_ta, na.rm = TRUE)
...
# For combined UNICEF
ta_unicef_se_income <- deltaMethod(
  ta_within_unicef,
  "dev_log_exp / b0 + 1",
  parameterNames = c("b0" = "dev_log_exp"),
  constants = list(
    b0 = mean(unicef_cluster_data$w_tobacco_alcohol, na.rm = TRUE)
  )
)

ta_unicef_se_price <- sqrt(vcov(ta_cluster_reg_unicef)[
  "log_price_ta",
  "log_price_ta"
]) /
  mean(unicef_clusters$mean_w_ta, na.rm = TRUE)
...
### Test for significant differences between SSU and UNICEF combined estimates ###

# Test for income elasticity difference
income_diff <- ta_income_elast_ssu - ta_income_elast_unicef
income_diff_se <- sqrt(ta_ssu_se_income$SE^2 + ta_unicef_se_income$SE^2)
income_z_stat <- income_diff / income_diff_se
income_p_value <- 2 * (1 - pnorm(abs(income_z_stat)))

# Test for price elasticity difference
price_diff <- ta_price_elast_ssu - ta_price_elast_unicef
price_diff_se <- sqrt(ta_ssu_se_price^2 + ta_unicef_se_price^2)
price_z_stat <- price_diff / price_diff_se
price_p_value <- 2 * (1 - pnorm(abs(price_z_stat)))
...
results_summary <- data.frame(
  Product = c(
    ...
    "Alcohol (SSU)",
    "Tobacco & Alcohol (SSU)",
    "Tobacco & Alcohol (UNICEF)"
  ),
  Income_Elasticity = round(
    c(
      tobacco_income_elast,
      alcohol_income_elast,
      ta_income_elast_ssu,
      ta_income_elast_unicef
    ),
    3
  ),
  Income_SE = round(
    c(
      tobacco_se_income$SE,
      alcohol_se_income$SE,
      ta_ssu_se_income$SE,
      ta_unicef_se_income$SE
    ),
    3
  ),
  Price_Elasticity = round(
    c(
      ...
      alcohol_price_elast,
      ta_price_elast_ssu,
      ta_price_elast_unicef
    ),
    3
  ),
  Price_SE = round(
    c(tobacco_se_price, alcohol_se_price, ta_ssu_se_price, ta_unicef_se_price),
    3
  )
)

# Print results
cat("\n=== DEATON METHOD ELASTICITY ESTIMATES ===\n")
print(results_summary)

cat("\n=== TEST FOR DIFFERENCES (SSU vs UNICEF Combined) ===\n")
cat("Income Elasticity Difference:", round(income_diff, 3), "\n")
cat("  Standard Error:", round(income_diff_se, 3), "\n")
cat("  Z-statistic:", round(income_z_stat, 3), "\n")
cat("  P-value:", round(income_p_value, 4), "\n\n")

cat("Price Elasticity Difference:", round(price_diff, 3), "\n")
cat("  Standard Error:", round(price_diff_se, 3), "\n")
cat("  Z-statistic:", round(price_z_stat, 3), "\n")
cat("  P-value:", round(price_p_value, 4), "\n")
...
cat("\n=== CLUSTER INFORMATION ===\n")
cat("SSU clusters:", nrow(ssu_clusters), "\n")
cat("UNICEF clusters:", nrow(unicef_clusters), "\n")

# Check price variation
cat("\nPrice variation (coefficient of variation):\n")
cat(
  "Tobacco prices:",
  sd(exp(ssu_clusters$log_price_tobacco)) /
    mean(exp(ssu_clusters$log_price_tobacco)),
  "\n"
)
cat(
  "Alcohol prices:",
  sd(exp(ssu_clusters$log_price_alcohol)) /
    mean(exp(ssu_clusters$log_price_alcohol)),
  "\n"
)
cat(
  "Combined prices (SSU):",
  sd(exp(ssu_clusters$log_price_ta)) / mean(exp(ssu_clusters$log_price_ta)),
  "\n"
)
cat(
  "Combined prices (UNICEF):",
  sd(exp(unicef_clusters$log_price_ta)) /
    mean(exp(unicef_clusters$log_price_ta)),
  "\n"
)
